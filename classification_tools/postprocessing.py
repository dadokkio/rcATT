...
def confidence_propagation_single(
    tactics_confidence_list, technique_name, technique_confidence_score
):
    """
    ...
    for tactic in clt.CODE_TACTICS:
        if not clt.TACTICS_TECHNIQUES_RELATIONSHIP_DF.loc[
            clt.TACTICS_TECHNIQUES_RELATIONSHIP_DF[tactic] == technique_name
        ].empty:
            lambdaim = 1 / (
                np.exp(
                    abs(technique_confidence_score - tactics_confidence_list[tactic])
                )
            )
            new_confidence_score = (
                new_confidence_score + lambdaim * tactics_confidence_list[tactic]
            )
        i = i + 1
...
def confidence_propagation(predprob_tactics, pred_techniques, predprob_techniques):
    ...
    tactics_confidence_df = pd.DataFrame(
        data=predprob_tactics, columns=clt.CODE_TACTICS
    )
    for j in range(len(predprob_techniques[0])):
        for i in range(len(predprob_techniques)):
            predprob_techniques_corrected[i][j] = confidence_propagation_single(
                tactics_confidence_df[i : (i + 1)],
                clt.CODE_TECHNIQUES[j],
                predprob_techniques[i][j],
            )
...
def hanging_node(
    pred_tactics, predprob_tactics, pred_techniques, predprob_techniques, c, d
):
    """
...
def hanging_node_threshold_comparison(
    pred_tactics,
    predprob_tactics,
    pred_techniques,
    predprob_techniques,
    known_pred_techniques,
    permutations,
):
    """
    ...
    for pl in permutations:
        f05list_temp = [pl]
        new_pred_techniques = hanging_node(
            pred_tactics,
            predprob_tactics,
            pred_techniques,
            predprob_techniques,
        ...
        f05list_temp.append(
            fbeta_score(
                known_pred_techniques, new_pred_techniques, beta=0.5, average="macro"
            )
        )
...
def find_best_post_processing(cmd):
    ...
    f05_NO = (
        []
    )  # list of f0.5 score for all techniques predictions sets without post-processing
    f05_HN = (
        []
    )  # list of f0.5 score for all techniques predictions sets with hanging node post-processing
    f05_CP = (
        []
    )  # list of f0.5 score for all techniques predictions sets with confidence propagation post-processing
    ...
    for index1, index2 in kf.split(reports, overall_ttps):
        ...
        overall_ttps_train, overall_ttps_test = (
            overall_ttps.iloc[index1],
            overall_ttps.iloc[index2],
        )
        ...
        f05_NO.append(
            fbeta_score(test_techniques, pred_techniques, beta=0.5, average="macro")
        )
        f05_HN.extend(
            hanging_node_threshold_comparison(
                pred_tactics,
                predprob_tactics,
                pred_techniques,
                predprob_techniques,
        ...
        CPres, _ = confidence_propagation(
    ...
    save_post_processing_comparison.extend(
        [
            best_HN,
            [min_prob_tactics, max_prob_tactics],
            [min_prob_techniques, max_prob_techniques],
        ]
    )
